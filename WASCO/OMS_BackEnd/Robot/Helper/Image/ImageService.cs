    ...
    public static class ImageService
    {
        public static Image ResizeImage(Image image, int width, int height, bool preserveAspectRatio = true) {
            ...
            if (preserveAspectRatio) {
                ...
                if (originalHeight > originalWidth) {//เป็นแนวตั้งให้เพิ่มความกว้างไปอีก 300 เพื่อรูปจะได้ไม่เล็กเกินไป
                    width = width + 300;
                    height = height + 300;
                }
                if (originalWidth <= width || originalHeight <= width) {//รูปต้นฉบับเล็กกว่า
                    width = originalWidth;
