...
namespace RobotWasm.Server.Controllers.Stock {
    ...
    async    public Task<I_BasicResult> PrintStockBalSale([FromBody] string data) {
     ...
     async   public Task<I_BasicResult> PrintStockPromotion([FromBody] string data) {
        ...
        [HttpPost("ListPrintStockBalSale")]
        public List<vw_ItemInfoWithPhotoAndStock> ListPrintStockBalSale([FromBody] string data) {
            var p = JsonSerializer.Deserialize<StockBalParam>(data, new JsonSerializerOptions {
                PropertyNameCaseInsensitive = true,
                ReferenceHandler = ReferenceHandler.Preserve
            });
        ...
        //[HttpPost("PrintStockBal")]
        //public List<vw_OSOLine> PrintStockBal([FromBody] string data) {
        //    var p = JsonSerializer.Deserialize<vw_OSOLine>(data, new JsonSerializerOptions {
        //        PropertyNameCaseInsensitive = true,
        //        ReferenceHandler = ReferenceHandler.Preserve
        //    });
        //    return StockBalanceService.PrintStockBal(p);
        //}
        [HttpPost("PrintStockBal")]
        async public Task<I_BasicResult> PrintStockBal([FromBody] string data) {
            I_BasicResult result = new I_BasicResult { Result = "ok", Message1 = "", Message2 = "" };
            ...
            result = await Task.Run(() => RobotWasm.Server.PrintOut.OMS.SO102.RunReport.Convert2PrintData(doc));

            if (result.Result == "ok") {
                result.Message2 = Globals.ApiPrintMasterBaseUrl + $"/PrintFile/OMS/SO102/ViewerSO102.aspx?id={result.Message2}";
            }
            return result;
        }
    }
