...
namespace RobotWasm.Shared.Data.DA {
    public class SOFuncService {
            ...
            try {
                ...
                if (input.Line.Count > 0) {
                    foreach (var l in input.Line) {
                        ...
                        l.VatAmt = Math.Round((l.TotalAmtIncVat * l.VatRate) / (100 + l.VatRate), 3, MidpointRounding.AwayFromZero);
                        //11.cal line nettotal amt inc vat
                        //l.TotalAmtIncVat = Math.Round(l.TotalAmt + l.VatAmt, 2, MidpointRounding.AwayFromZero);
                        l.TotalAmt = Math.Round(l.TotalAmtIncVat - l.VatAmt, 3, MidpointRounding.AwayFromZero);
        ...
        public static I_SODocSet ClearPendingLine(I_SODocSet input) {
            try {
        ...
        public static I_SODocSet ClearPendingLot(I_SODocSet input) {
            try {
        ...
        public static I_SODocSet ProCal(I_SODocSet doc, Promotions CurrentPro) {
            try {
                if (CurrentPro.PatternID == "P000") {
                    doc.LineActive.IsComplete = true;
                }
                if (CurrentPro.PatternID == "P100") {
                    if (doc.LineActive!=null) {
