...
namespace RobotWasm.Client.Data.DA.UserGroup {
    public class UserService {
        ...
        async public Task<I_BasicResult> ChangePassword(string username, string cur_pass, string newpass) {
            I_BasicResult? output = new I_BasicResult { Result = "ok", Message1 = "", Message2 = "" };
            try {
                ResetPasswordRequest data = new ResetPasswordRequest { User = username, password = cur_pass, NewPassword = newpass };
                string strPayload = JsonSerializer.Serialize(data);
        ...
        async public Task<IEnumerable<UserInfo>> ListUserInfo(string search) {
            IEnumerable<UserInfo>? output = new List<UserInfo>();
            try {
                UserParam p =new UserParam { Search=search };
                string strPayload = JsonSerializer.Serialize(p);
                string url = $"api/User/ListUserInfo";
