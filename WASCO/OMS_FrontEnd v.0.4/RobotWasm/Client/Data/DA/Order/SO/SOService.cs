...
namespace RobotWasm.Client.Data.DA.Order.SO {
    public class SOService {
        ...
        async public void SetSessionSearchText(string text) { 
            await _sessionStorage.SetItemAsync("so_search_item", text);
        }
        async public Task<string>? GetSessionSearchText() { 
            string  text = await _sessionStorage.GetItemAsync<string>("so_search_item");
            text = text == null ? "" : text;
            return text;
        }
        #endregion
