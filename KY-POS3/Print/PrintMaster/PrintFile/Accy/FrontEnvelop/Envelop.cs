...
using static PrintMaster.PrintFile.Accy.FrontEnvelop.RunReportFrontEnvelop;

namespace PrintMaster.PrintFile.Accy.FrontEnvelop {
    public partial class Envelop : DevExpress.XtraReports.UI.XtraReport {
        public Envelop(FrontEnvelopSet data, string UseFor, string typeprint, string copyno) {
            InitializeComponent();
            LoadData(data, UseFor, typeprint, copyno);
        }

        private void LoadData(FrontEnvelopSet d, string UseFor, string typeprint, string copyno) {

            lblCustVenName.Text = d.head.NameTh1;
            if (d.head.BillAddr2 != "") { 
