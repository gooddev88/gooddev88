...
namespace Robot.Data.DA {
    public class MyUserGroupService {
            ...
            try {
                using (GAEntities db = new GAEntities()) {
                    ...
                    foreach (var c in rcoms) {//สร้าง group supperman ให้กับทุก Rcom
                        var ugroup = NewUsergroup(username, "");
        ...
        public static UserGroupInfo NewUsergroup(string username, string rcom) {
        ...
        public I_userGroupSet NewTransaction(string username, string rcom) {
            I_userGroupSet n = new I_userGroupSet();
            n.Group = NewUsergroup(username, rcom);
