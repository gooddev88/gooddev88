...
namespace Robot.Service.FileGo {
    public class FileGo {
      ...
      async  public   Task<string> GetThumbBase64(string rcom, string com, string doctype, string docId) {
            string filebase64 = "";
         
            using (GAEntities db = new GAEntities()) {
                var f = db.vw_XFilesRef.Where(o => o.IsActive && o.RCompanyID == rcom && o.CompanyID == com && o.DocID == docId && o.DocType == doctype && o.PathType == "FILEGO").OrderByDescending(o => o.CreatedDate).FirstOrDefault();
                if (f != null) {
                  var  url = f.RootUrlPublic + @"/api/xfiles/XFilesService/GetThumbBase64/" + f.FileID;
                 //   filebase64 = await Task.Run(() => _clientService.GetAllAsync<string>(url));
                    var query = await Task.Run(() => _clientService.GetAllAsync<string>(url));
                  
                    if (query.StatusCode != "OK") {
                        return null;
                    } else {
                        filebase64= JsonSerializer.Deserialize<string>((string)query.Result, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });                    
                    }
                }
            }
            return filebase64;
        ...
        async public Task<I_BasicResult> DeleteOldFileAfterSave(string rcom, string com, string doctype, string docid, string deleted_by) {
            ...
            try {
                using (GAEntities db = new GAEntities()) {
                    ...
                    var query = await Task.Run(() => _clientService.Post<I_BasicResult>($"{latest_file.RootUrl}/api/xfiles/XFilesService/DeleteFile/", delete_file_ids));
                    if (query.StatusCode == "OK") {
        ...
        async public Task<I_BasicResult> DeleteFile(string rcom, string com, string doctype, string docid, string deleted_by) {
            ...
            try {
                using (GAEntities db = new GAEntities()) {
                    ...
                    var query = await Task.Run(() => _clientService.Post<I_BasicResult>($"{f.RootUrl}/api/xfiles/XFilesService/DeleteFile/", file_ids));
                    if (query.StatusCode == "OK") {
