...
namespace RobotWasm.Client.Data.DA.Xfiles {
    public class FileGoService { 
        ...
        async public Task<I_BasicResult> DeleteFileByFileIdPostgres(string rcom, string? com, string doctype, string docid, string fileid, string deleted_by) {
            ...
            I_BasicResult? output = new I_BasicResult { Result = "ok", Message1 = "", Message2 = "" };
            try {
                string url = $"api/FileGo/DeleteFileByFileIdPostgres?rcom={rcom}&com={com}&doctype={doctype}&docid={docid}&fileid={fileid}&deleted_by={deleted_by}";
        ...
        async public Task<vw_xfile_ref> GetFileIdPostgres(string rcom, string? com, string doctype, string docid)
        {
            com = com == null ? "" : com;
            vw_xfile_ref? output = new vw_xfile_ref();
            try
            {
                var res = await _http.GetAsync($"api/FileGo/GetXfileRefPostgres?rcom={rcom}&com={com}&doctype={doctype}&docid={docid}");
                output = JsonSerializer.Deserialize<vw_xfile_ref>(await res.Content.ReadAsStringAsync(), new JsonSerializerOptions
                {
                    PropertyNameCaseInsensitive = true,
                    ReferenceHandler = ReferenceHandler.Preserve
                });
        ...
        async public Task<xfiles> GetFileInBytePostgres(string rcom, string? com, string doctype, string docId)
        {
            ...
            try
            {
                var res = await _http.GetAsync($"api/FileGo/GetFileInBytePostgres?rcom={rcom}&com={com}&doctype={doctype}&docId={docId}");
                output = JsonSerializer.Deserialize<xfiles>(await res.Content.ReadAsStringAsync(), new JsonSerializerOptions
                {
                    PropertyNameCaseInsensitive = true,
                    ReferenceHandler = ReferenceHandler.Preserve
                });
