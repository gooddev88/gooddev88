...
namespace RobotWasm.Server.Data.DA.Login {
    public class LogInCIMSService {
        ...
        public static LoginSet Login(string username, string password, string appid, string rcom = "") {
            ...
            try {
                using (cimsContext db = new cimsContext()) {
                    ...
                    var query = db.user_info.Where(o => o.username.ToLower() == username && o.is_active == 1).FirstOrDefault();
        ...
        public static I_BasicResult UpdatePasswordNew(string? username, string password, string newPassword) {
            ...
            try {
                using (cimsContext db = new cimsContext()) {
                    password = Hash.hashPassword("MD5", password);
                    newPassword = Hash.hashPassword("MD5", newPassword);
                    var u = db.user_info.Where(o => o.username == username).FirstOrDefault();
                    if (u == null) {
                        result.Result = "fail";
        ...
        public static List<vw_PermissionInMenu> ListUserInMenuDisplay(List<vw_PermissionInMenu> input) {
            ...
            try {
                ...
                result = input.Where(o => o.IsOpen == true).GroupBy(x => new { x.Username, x.MenuID, x.GroupID, x.TypeID, x.MenuName, x.MenuDesc1, x.MenuDesc2, x.Url, x.Icon, x.GroupSort,x.AppID,x.menu_sort })
        ...
        #endregion
        #region  mapping
        public static UserInfo MappingUserInfo(user_info x) {
