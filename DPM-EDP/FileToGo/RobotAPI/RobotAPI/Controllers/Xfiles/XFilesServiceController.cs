...
namespace RobotAPI.Controllers.Xfiles {
    ...
    public class XFilesServiceController : BaseApiController {
        ...
        [AllowAnonymous]
        [HttpGet("GetThumbBase64/{file_id}")]
        public string GetThumbBase64(string file_id) {
            string result = "";
            try {
                var file = XFilesService.GetFile(file_id, true);
                result= JsonSerializer.Serialize(  Convert.ToBase64String(file.data_thumb));
                // result = Convert.ToBase64String(file.data_thumb);
            } catch {
                Response.StatusCode = 400;
            }
            return result;
